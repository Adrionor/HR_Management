...
def registro_candidato_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            puesto_seleccionado = form.cleaned_data.get('puesto_de_interes')

            # Si el valor es un número, es un Puesto real
            if puesto_seleccionado and puesto_seleccionado.isdigit():
                candidato.puesto_de_interes_id = int(puesto_seleccionado)
            # Si seleccionó 'Prácticas' o la opción vacía, el puesto de interés es Nulo.
            else:
                candidato.puesto_de_interes = None

            # (Opcional) Podemos guardar la opción de prácticas en otro campo si quisiéramos
            # if puesto_seleccionado == 'PRACTICAS':
            #     candidato.notas_adicionales = "Interesado en Prácticas Profesionales"

            candidato.save()
            # Guardamos las relaciones Many-to-Many si las tuvieras
            form.save_m2m()
...
def reportes_view(request):
    # ... (El cálculo de los KPIs no cambia) ...

    # --- LÓGICA ACTUALIZADA PARA LA TABLA DE ESTATUS ---

    # 1. Obtenemos el mapa de traducción como antes
    estatus_map = dict(Proceso.Estatus.choices)

    # 2. Hacemos la consulta a la base de datos como antes
    procesos_por_estatus_query = Proceso.objects.values('estatus_proceso').annotate(total=Count('id')).order_by(
        '-total')

    # 3. ¡NUEVO! Creamos una nueva lista con los datos ya traducidos
    tabla_procesos_por_estatus_traducida = []
    for item in procesos_por_estatus_query:
        codigo_estatus = item['estatus_proceso']
        tabla_procesos_por_estatus_traducida.append({
            'estatus_legible': estatus_map.get(codigo_estatus, codigo_estatus),  # Usamos el mapa
            'total': item['total']
        })
    # --- FIN DE LA LÓGICA ACTUALIZADA ---

    contexto = {
        # ... (tus otros KPIs y datos de tablas no cambian)

        # Pasamos la nueva lista ya traducida a la plantilla
        'tabla_procesos_por_estatus': tabla_procesos_por_estatus_traducida,

        # Ya no necesitamos pasar 'estatus_map'
    }
...
def portal_gerente_view(request):
    ...
    mis_solicitudes = Puesto.objects.filter(solicitado_por=request.user).select_related('asesora_encargada').annotate(num_procesos=Count('proceso'))
...
from .forms import SolicitudPuestoForm, RegistroCandidatoForm, PublicacionForm, PerfilDePuestoForm
from .models import Candidato, Puesto, Proceso, RegistroActividad, Publicacion, PerfilDePuesto, Marca


# ... (tus funciones de ayuda para permisos no cambian) ...

# --- VISTA BOLSA DE TRABAJO CORREGIDA ---
@login_required
@user_passes_test(puede_ver_operaciones, login_url=reverse_lazy('acceso_denegado'))
@no_cache_page
def bolsa_trabajo_view(request):
    if request.method == 'POST':
        candidato = get_object_or_404(Candidato, id=request.POST.get('candidato_id'))
        puesto = get_object_or_404(Puesto, id=request.POST.get('puesto_id'))
        nuevo_proceso = Proceso.objects.create(candidato=candidato, puesto=puesto, asesora_asignada=request.user,
        ...
        RegistroActividad.objects.create(usuario=request.user,
                                         ...
                                         tipo_objeto="Proceso", id_objeto=nuevo_proceso.id)
        messages.success(request, f"Candidato '{candidato.nombre_completo}' asignado al proceso exitosamente.")
        if request.POST.get('puesto_fijo'):
            return redirect('mis_vacantes')
        return redirect('bolsa_trabajo')

    puesto_seleccionado_id = request.GET.get('puesto')
    puesto_seleccionado = None
    if puesto_seleccionado_id:
        puesto_seleccionado = get_object_or_404(Puesto, id=puesto_seleccionado_id)

    estatus_finalizados = [Proceso.Estatus.NO_APROBADO_PSICO, Proceso.Estatus.EN_BOLSA,
                           Proceso.Estatus.CONTRATADO_CERRADO]
    candidatos_ocupados_ids = Proceso.objects.exclude(estatus_proceso__in=estatus_finalizados).values_list(
        'candidato_id', flat=True)
    candidatos_disponibles = list(Candidato.objects.exclude(id__in=candidatos_ocupados_ids).order_by('-fecha_registro'))

    un_ano_atras = timezone.now() - timezone.timedelta(days=365)
    for candidato in candidatos_disponibles:
        ultimo_proceso_finalizado = Proceso.objects.filter(candidato=candidato,
                                                           estatus_proceso__in=estatus_finalizados).order_by(
            '-fecha_inicio_etapa').first()
    ...
    # Filtramos los puestos no solo por estar abiertos, sino también por el usuario actual
    puestos_de_la_asesora = Puesto.objects.filter(
        asesora_encargada=request.user,
        esta_abierto=True
    )

    contexto = {'candidatos': candidatos_disponibles, 'mis_puestos': puestos_de_la_asesora,
                'puesto_seleccionado': puesto_seleccionado}
    return render(request, 'reclutamiento/bolsa_trabajo.html', contexto)
# reclutamiento/views.py
#perfil de puesto
...
def mis_vacantes_view(request):
    vacantes_sin_iniciar = Puesto.objects.filter(
        asesora_encargada=request.user,
        esta_abierto=True
    ).annotate(
        num_procesos=Count('proceso')
    ).filter(
        num_procesos=0
    ).select_related('perfil_detallado') # <-- AÑADIMOS ESTO

    contexto = {
        'vacantes': vacantes_sin_iniciar
    }
